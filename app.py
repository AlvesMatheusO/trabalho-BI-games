...
NUMERIC_COLS = ["total_sales", "critic_score", "na_sales", "pal_sales", "jp_sales", "other_sales"]
REQUIRED_COLS = ["title", "genre", "publisher", "developer", "critic_score", "total_sales",
...
def load_dataset() -> pd.DataFrame:
    ...
    print(f"[INFO] {len(df):,} linhas — {CSV_PATH.name}")
    ...
    df["console"] = df["console"].fillna("Multi").astype(str) if "console" in df.columns else "Multi"
...
def genres():
    return jsonify(df["genre"].dropna().sort_values().unique().tolist())
...
def top_sum(dataframe, group_col, value_col, limit=10) -> dict:
    data = (dataframe.groupby(group_col)[value_col]
            .sum().sort_values(ascending=False).head(limit).round(2))
    return {str(k): float(v) for k, v in data.items()}


def _explode_consoles(dataframe, limit=10) -> pd.Series:
    """
    A coluna 'console' no CSV consolidado contém uma lista separada por ' | '
    (ex: 'PS3 | PS4 | X360'). Explode para somar vendas por plataforma real.
    """
    exploded = (
        dataframe[["console", "total_sales"]]
...
def top_consoles_dict(dataframe) -> dict:
    """Formato {console: vendas} — usado no gráfico de barras 'Top Consoles'."""
    top = _explode_consoles(dataframe, limit=10)
    return {str(k): float(v) for k, v in top.items()}


def console_market_share(dataframe) -> dict:
    """Formato {labels, values} — usado no gráfico donut 'Market Share'."""
    top = _explode_consoles(dataframe, limit=8)
    return {"labels": top.index.astype(str).tolist(), "values": top.round(2).tolist()}


def regional_distribution(dataframe) -> dict:
    regions = {
...
def critic_histogram(dataframe) -> dict:
    cs = dataframe["critic_score"]
    return {
...
def strategic_quadrant(dataframe) -> list:
    grouped = (
        dataframe.groupby("genre")
        .agg(total_sales=("total_sales","sum"), avg_critic=("critic_score","mean"), games=("title","count"))
        .reset_index().sort_values("total_sales", ascending=False).head(12)
    )
    return [{"x": round(float(r["avg_critic"]),2), "y": round(float(r["total_sales"]),2),
             "z": int(r["games"]), "name": r["genre"]} for _, r in grouped.iterrows()]


def radar_strategy(dataframe) -> dict:
    grouped = (
        dataframe.groupby("genre")
        .agg(total_sales=("total_sales","sum"), avg_critic=("critic_score","mean"),
             games=("title","count"), na=("na_sales","sum"), pal=("pal_sales","sum"),
    ...
    for _, row in grouped.iterrows():
        ...
        series.append({"name": row["genre"], "data": [
            round(float(sales_score),2), round(float(critic_score),2),
            round(float(global_pres),2), round(float(volume_score),2), round(float(opportunity),2),
        ]})
    return {"categories": ["Vendas","Crítica","Presença Global","Volume de Jogos","Oportunidade"], "series": series}


def publisher_ranking(dataframe) -> dict:
    data = (
        dataframe.groupby("publisher")
        .agg(total_sales=("total_sales","sum"), avg_critic=("critic_score","mean"), games=("title","count"))
        .reset_index().sort_values("total_sales", ascending=False).head(10)
    )
    return {"labels": data["publisher"].tolist(), "sales": data["total_sales"].round(2).tolist(),
            "critic": data["avg_critic"].round(2).tolist(), "games": data["games"].astype(int).tolist()}


def scatter_points(dataframe) -> list:
    valid = (
        dataframe[["critic_score","total_sales","title","genre"]]
        .dropna().query("critic_score >= 0 and total_sales >= 0")
        .sort_values("total_sales", ascending=False).head(1200)
    )
    return valid.to_dict(orient="records")


def generate_insights(dataframe, genre: str) -> list:
    if dataframe.empty:
    ...
    if not top_publisher.empty:
        insights.append(f"A publisher líder é {top_publisher.index[0]}, com {top_publisher.iloc[0]:.1f}M em vendas totais.")

    insights.append(
        "A nota crítica média está em um nível saudável (≥ 7.5), sugerindo boa aceitação qualitativa do catálogo."
        if avg_critic >= 7.5 else
        "A nota crítica média está abaixo de 7.5. Estratégias de melhoria de qualidade podem ser importantes."
    )
    if total_games > 0:
        spg = total_sales / total_games
        insights.append(
            f"Média de {spg:.2f}M vendas por jogo — "
            + ("alto potencial comercial por título." if spg >= 1 else "mercado mais pulverizado ou nichado.")
        )
    return insights
...
def summary():
    ...
    consoles_unique = int(
        filtered["console"].str.split(r"\s*\|\s*").explode().nunique()
    )
    ...
    payload = {
        "globalMetrics": {
            ...
            "uniqueConsoles": consoles_unique,
        },
        # ✅ CORRIGIDO: usa console real (explodido de lista) — não mais gênero
        "topConsoles":    top_consoles_dict(filtered),
