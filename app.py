...
def radar_strategy(dataframe):
    ...
    for _, row in grouped.iterrows():
        ...
        sales_score = (
            row["total_sales"] / max_sales * 100
            if max_sales > 0
            else 0
        )

        critic_score = (
            row["avg_critic"] / 10 * 100
            if row["avg_critic"] > 0
            else 0
        )

        volume_score = (
            row["games"] / max_games * 100
            if max_games > 0
            else 0
        )
...
def scatter_points(dataframe):
    valid = (
        dataframe[
            ["critic_score", "total_sales", "title", "genre"]
        ]
        .dropna()
...
def generate_insights(dataframe, genre):
    ...
    if genre == "all" and not top_genre.empty:
        insights.append(
            f"O gênero com maior volume de vendas é {top_genre.index[0]}, com {top_genre.iloc[0]:.1f}M em vendas."
        )

    if not top_console.empty:
        insights.append(
            f"O console mais forte no filtro atual é {top_console.index[0]}, somando {top_console.iloc[0]:.1f}M em vendas."
        )

    if not top_publisher.empty:
        insights.append(
            f"A publisher líder é {top_publisher.index[0]}, indicando uma possível referência estratégica para posicionamento."
        )

    if avg_critic >= 7.5:
        insights.append(
            "A nota crítica média está em um nível saudável. Isso sugere boa aceitação qualitativa do catálogo analisado."
        )
    else:
        insights.append(
            "A nota crítica média está abaixo de 7.5. Estratégias de melhoria de qualidade e polimento podem ser importantes."
        )
    ...
    if total_games > 0:
        ...
        if sales_per_game >= 1:
            insights.append(
                f"A média de vendas por jogo é de {sales_per_game:.2f}M, indicando alto potencial comercial por título."
            )
        else:
            insights.append(
                f"A média de vendas por jogo é de {sales_per_game:.2f}M, sugerindo mercado mais pulverizado ou nichado."
            )
